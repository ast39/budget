...
namespace App\Packages\Finance;

use App\Packages\Finance\Credit\RequestData;
...
trait DetailsTrait {
     ...
     * @return array
     */
    public static function monthlyStatement(
        RequestData $credit
    ): array
    {
        $details  = [];
        ...
        for ($i = 1; $i <= $credit->period; $i++) {
            ...
            $current_percent = ceil(self::getPercentByPeriod($old_payment_date, $current_payment_date, $inset_balance, $credit->percent));
            ...
            $current_payment_date = self::plusMonth($current_payment_date);
        }
     ...
     * @return array
     */
    public static function monthlyStatementDiff(
        RequestData $credit
    ): array
    {
        $details  = [];
     ...
     * @return int
     */
    protected static function getMonth(int $start_date, int $payment_num): int
    {
        $new_date = $start_date;
        for ($i = 1; $i <= $payment_num; $i++) {
            $new_date = self::plusMonth($new_date);
        }
     ...
     * @return int
     */
    protected static function plusMonth(int $time): int
    {
     ...
     * @return int
     */
    protected static function daysInYear(int $pay_date): int
    {
     ...
     * @return float
     */
    protected static function getPercentByPeriod(int $from, int $to, int $mount, float $percent): float
    {
        $seconds_in_day = 3600 * 24;
        $percent_amount = 0;

        do {
            $percent_amount += $mount * $percent / 100 / self::daysInYear($from);
            $from += $seconds_in_day;
