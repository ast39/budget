...
class Helper {
    ...
    public static function daysToPayment(int $first_payment_date, int $payments_count): int
    {
        ...
        while($payment_date <= $time) {
            ...
            $payment_date = Manager::shiftMonth($payment_date);
        }
        ...
        $payment_date = Manager::shiftMonth($payment_date, $shift);

        # День платежа сегодня
        if ($today == $payment_day) {
            return 0;
        } else {
            return floor(($payment_date - $time) / Manager::DAY);
        }
    }
